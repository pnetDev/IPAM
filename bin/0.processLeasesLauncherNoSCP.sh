...
clear
echo ""
echo "This version does not change dhcp config."
echo "This version runs on the database on 10.1.1.24"
echo "#########################################"
...
## Comment these in if routes change on CCR1
...
#grep -v "#" $parsedFiles/CCR1Routes.terse.raw.txt  | grep -v "/32" > $parsedFiles/CCR1Routes.terse.txt  ## We don't want /32 routes.

## Get the lease files from all DHCP servers
#echo "NOT GETTING LEASES FILES"
...
#scp root@10.1.1.24://var/db/dhcpd.leases $dhcp/dhcpd.leases24 
#scp root@10.1.1.24://var/db/dhcpd.leases $dhcp/dhcpd.leases24$currDate
...
#cp $dhcp/dhcpd.conf24 $dhcp/dhcpd.conf24$currDate
...
#scp root@10.1.1.24://etc/dhcpd.conf $dhcp/dhcpd.conf24 > /dev/null
...
#scp root@10.1.1.24://etc/dhcp/*.dyn /$dhcp/
...
# Strange issue doing a normal 'cat' on the leases files so doing it these way, horrible way to do it I know.
more $dhcp/dhcpd.leases66 > $parsedFiles/dhcpd.leasesAllServers
$scriptDir/parseLeasesCM.csv.py > $parsedFiles/parsed66
more $dhcp/dhcpd.leases68 > $parsedFiles/dhcpd.leasesAllServers
$scriptDir/parseLeasesCM.csv.py > $parsedFiles/parsed68
more $dhcp/dhcpd.leases75 > $parsedFiles/dhcpd.leasesAllServers
$scriptDir/parseLeasesCM.csv.py > $parsedFiles/parsed75
#more $dhcp/dhcpd.leases24 > $parsedFiles/dhcpd.leasesAllServers
#$scriptDir/parseLeasesCM.csv.py > $parsedFiles/parsed24
more $dhcp/dhcpd.leases233 > $parsedFiles/dhcpd.leasesAllServers
$scriptDir/parseLeasesCM.csv.py > $parsedFiles/parsed233

# Calculate the active leases for a handy report.
leased66=$(wc $parsedFiles/parsed66 | awk {'print ($1) } ')
leased68=$(wc $parsedFiles/parsed68 | awk {'print ($1) } ')
leased75=$(wc  $parsedFiles/parsed75 | awk {'print ($1) } ')
#leased24=$(wc  $parsedFiles/parsed24 | awk {'print ($1) } ')
leased233=$(wc  $parsedFiles/parsed233 | awk {'print ($1) } ')
...
#echo 24 = $leased24 >> $Log
echo 233 = $leased233 >> $Log
echo "=========================" >> $Log

#total=$((leased66+leased68+leased75+leased24))
...
#cat parsed66 parsed68 parsed75 parsed24 > dhcpd.leasesAllServersParsed
cat  $parsedFiles/parsed66  $parsedFiles/parsed68  $parsedFiles/parsed75  $parsedFiles/parsed233 >  $parsedFiles/dhcpd.leasesAllServersParsed

echo "Parsing dhcpd.confAllServers and writing to dhcpd.confParsed." >> $Log

## CM Testing to see can we parse classes
...
cd $dhcp
rename syntax rename .classes .classes$currDate *.classes
...
#scp root@10.1.1.24://etc/dhcp/*.classes    $dhcp/
...
$scriptDir/1.updateDatabaseWithCCR_RoutesArea51.py
sleep 2

## Flag the persistentStatic IPs so these will never be recycled.
$scriptDir/1.5.updatePersistentStaticArea51.py

## Update database with dhcpd.conf shared-networks
echo Updating database dhcpd.conf shared-networks.
#$scriptDir/2.readDhcpdConfUpdateDatabaseArea51.Pytricia.py
$scriptDir/2.1.readDhcpdConfUpdateDatabaseArea51.Pytricia.py ## CM 180831 New version, now subnets don't need to hardcoded!
#$scriptDir/2.readDhcpdConfUpdateDatabaseArea51.py
sleep 2

## Update database IPs which are to be set to TYPE Dynamic
echo Updating database with dynamic ranges
$scriptDir/2.5.parseDynamicArea51.py
sleep 2

## Update database with leases. Reassign any leases which are part of a reserved range. First 4 usable addresses of a subnet.
echo Updating database with leases.
$scriptDir/3.readLeaseUpdateDatabaseArea51.py
sleep 2

#echo Exit after 3.readLeaseUpdateDatabaseArea51.py
#exit

## CM 180802 Commenting out until bug fixed.
## Check for IPs leased yesterday and not today
#echo "Checking for IPs leased yesterday and not today."
$scriptDir/3.5.compareTodaysLeasesToYesterdayArea51.py
sleep 2
...
$scriptDir/3.6.convertDynamicLeasesArea51.py
sleep 5

## Generating Classes files.
echo "Generating Classes Files."
$scriptDir/4.queryDatabaseToGenerateClassesArea51.py
...
if [ $result == "None" ] ; then 
	...
	echo "Duplicates found, now exiting"
	exit
fi

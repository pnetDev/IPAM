...
def parse_hardware(raw_str):
		...
		print ""
		print "Parse error in hardware"
		print ""
                raise Exception('Parse error in hardware')

#def parse_option(raw_str):
#        tokens = raw_str.split()
#
#        if len(tokens) == 2:
#            if tokens[0] == 'agent.remote-id':
#                return tokens[1]
#            else:
#                return 'otherOption' 
#
#        else:
#                raise Exception('Parse error in option')
        ...
        for line in leases_file:
                ...
                if key == 'lease':
                        if not in_lease:
                                ...
                                lease_rec['agent.remote-id'] = None
                                in_lease = True
                        ...
                        if in_lease:
                                ...
                                lease_rec[option_type] = tokens[2].rstrip(';')
...
#print('| IP Address      | MAC Address       | Expires (days,H:M:S) | Client Hostname | agent.remote-id')
#print('+-----------------+-------------------+----------------------+-----------------')

for lease in report_dataset:
	#print "Debug: Lease " + str(lease)
        print(format(lease['ip_address']) + ',' + \
                format(lease['hardware'], '<10') + ',' + \
                format(str((lease['starts']) if lease['ends'] != 'never' else 'never'), '>20') + ',' + 
                format(lease['agent.remote-id'],) )
